...
impl<T> RangeSet<T> {
    ...
    /// [`RangeSet::difference_iter`], which is called here internally. However,
    /// this may be slightly faster if you need a set.
    pub fn difference<'a>(&'a self, other: &'a Self) -> RangeSet<&'a T>
    where
        T: Ord,
    {
...
/// Set Difference
impl<'a, T: Ord + Clone> core::ops::Sub<&'a RangeSet<T>> for &'a RangeSet<T> {
    type Output = RangeSet<&'a T>;

    fn sub(self, rhs: &'a RangeSet<T>) -> RangeSet<&'a T> {
        self.difference(rhs)
    }
}
...
/// An iterator representing the difference between two [`RangeSet`]s
///
/// This struct is generated by [`RangeSet::difference_iter`]
